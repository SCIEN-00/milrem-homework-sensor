...
class Value(Union):
    _fields_ = [(value.name, ctype)
                for value, ctype in ValueTypeToCtype.items()]
    ...
    def setup_sensor_sending(self):
        ...
        for x in range(NR_SENSORS):
            ...
            self.loop.call_later(interval, self.send_packet,
                                 x, value_type, max_value, interval)

    def datagram_received(self, data, addr):
        self.logger.debug(
            "Received %d bytes of data from %s, discarding", len(data), addr)

    def send_packet(self, sensor_id, value_type, max_value, interval=1):
        if not self.transport:
            self.logger.warning(
                "UDP transport is not up yet, ignoring packet send")
            return
        ...
        self.loop.call_later(interval, self.send_packet,
                             sensor_id, value_type, max_value)
